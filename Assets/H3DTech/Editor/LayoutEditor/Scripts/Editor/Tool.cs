...
namespace H3DEditor
{
    ...
    public class LayoutTool
    {
        ...
        public static GameObject CreateBackground(bool for_edit)
        {
            ...
            background.GetComponent<Renderer>().sharedMaterial = new Material(Shader.Find("Unlit/Transparent Colored"));
        ...
        public static Dictionary<GameObject, string> ProcessSubPrefabWhenImport(GameObject root)
        {
            UIElement rootUIElement = root.GetComponent<UIElement>();
            if (rootUIElement == null)
            {
                Debug.Log("Root Obj Has No UIElement Component");
                return null;
            }

            Dictionary<GameObject, string> d = new Dictionary<GameObject, string>();
            UIPrefabNode[] coms = root.GetComponentsInChildren<UIPrefabNode>();
            foreach (UIPrefabNode node in coms)
            {
                GameObject objUIPrefab = node.gameObject;
                if(objUIPrefab.transform.childCount != 1)
                {
                    Debug.Log("Wrong Child Count For UIPrefabNode");
                    continue;
                }
                GameObject go = objUIPrefab.transform.GetChild(0).gameObject;
                objUIPrefab.transform.DetachChildren();
                ...
                string sFullpath = ui.FullPathName;
                string sPrefabPath = "";
                bool bFind = false;
                foreach (UIElement.SubPrefabPair pp in rootUIElement.SubPrefabInfos)
                {
                    if(pp.sGameObjPath == sFullpath)
                    {
                        sPrefabPath = pp.sPrefabPath;
                        bFind = true;
                        break;
                    }
                }
                ...
                if (bFind)
                {
                    if(sPrefabPath.Length == 0)
                    {
                        Debug.Log("Find Prefab Path but is empty");
                        continue;
                    }
                    d.Add(go, sPrefabPath);
                }
                else
                {
                    Debug.Log("Prefab Path not find!");
                    continue;
                }
            }
        ...
        private static bool CheckSubPrefab(GameObject root)
        {
            return CheckSubPrefabImpl(root, 0);
        }
        ...
        {
            string back_texture = EditorUtility.OpenFilePanel("��ѡ�񱳾�ͼ", EditorPrefs.GetString("BackTextureFolder"), "png");
        ...
        {
            string layout = EditorUtility.OpenFilePanel("��ѡ��Layout�ļ�", EditorPrefs.GetString("LayoutFolder"), "layout");
        ...
        private static void LoadSpecialInfo(UILabel go_ui, UILabel ui)
        {
            ...
            go_ui.effectColor = ui.effectColor;
            go_ui.maxLineCount = ui.maxLineCount;
            go_ui.trueTypeFont = ui.trueTypeFont;
            go_ui.fontSize = ui.fontSize;
            go_ui.fontStyle = ui.fontStyle;
            go_ui.color = ui.color;
        }
    ...
    public class ConfigTool
    {
        ...
        static string back_texture = "Assets/H3DTechConfig/LayoutEditor/back_texture.png";
