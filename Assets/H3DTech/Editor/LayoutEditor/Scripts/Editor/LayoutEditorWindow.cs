...
public class LayoutEditorWindow : EditorWindow
{
    ...
    {
        isInit = true;
        ConfigTool.Instance.LoadConfig();
    ...
    public void SyncUITreeLockForSubPrefab(List<UIElement> vUI)
    {
        SyncUITreeLockForSubPrefabImpl(vUI, uiTree.GetRootNode(0));
        RequestRepaint();
    }
    private void SyncUITreeLockForSubPrefabImpl(List<UIElement> vUI, TreeNode node)
    {
        UIElement element = node.DataKey as UIElement;
        if (element != null && vUI.Contains(element))
        {
            if(node.ToggleList != null)
            {
                node.ToggleList["锁定"] = element.Lock;
                if (!element.Lock)
                {
        ...
        {
            SyncUITreeLockForSubPrefabImpl(vUI, subNode);
        }
    }
