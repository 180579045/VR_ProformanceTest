...
public class SpecialEffectMenuItems
{
    ...
    [MenuItem("H3D/特效编辑/创建/创建特效动画片段调试物体")]
    static void CreateSpecialEffectAnimClipDebugObject()
    {
        GameObject speAnimClipDebugObj = GameObject.Find("_SPE_ANIMCLIP_DEBUG_OBJ") as GameObject;
        if (speAnimClipDebugObj != null)
        {
            EditorUtility.DisplayDialog("警告", "您已经创建一个\"特效调试物体\"无需再次创建!", "确认");
            return;
        }
        speAnimClipDebugObj = new GameObject("_SPE_ANIMCLIP_DEBUG_OBJ");
        speAnimClipDebugObj.AddComponent<SpecialEffectAnimClipDebugObject>();
    }
