Shader "H3D/InGame/SceneDynamicObjects/Unlit/Reflective(two side transparent)" {
	Properties 
	{
		_MainTex ("主纹理(RGB)", 2D) = "white" {} 
		_TintColor("主色调",Color) = (1.0,1.0,1.0,1.0)
		_ReflTex ("反射纹理", 2D) = "white" {}
		_ReflPowerTex("反射率纹理",2D) = "white"{}
		...
		_SHLightingScale("Ambient LightProbe Effect",Range(0.0,3.0)) = 1
	...
	SubShader {
		...
		Pass {  
				...
				fixed4 frag (v2f i) : SV_Target
				{
					...
					c.rgb    = lerp(mc.rgb *_TintColor ,refColor.rgb*_ReflColor ,_ReflPower * refPowerColor.a);
